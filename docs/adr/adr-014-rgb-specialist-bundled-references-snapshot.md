...
Task 6 is closed: `skills/specialist/references/rgb-system.md` is now
regenerated by `make generate` (`internal/components/tooling.GenerateSkillSnapshot`,
reading the input list from `skills/specialist/config.yaml`'s
`sources.bundled_snapshot_inputs`) and drift-checked in CI
(`scripts/ci/check-generated-drift.sh`), which `deploy` already depends on.
The "accepted cost" above — drift between this file and `docs/core/**` until
Task 6 landed — no longer applies: the file is a generated artifact now, not
hand-maintained, and a stale snapshot blocks deployment instead of
publishing silently.
